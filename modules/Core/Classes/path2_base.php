...
class path2_base {
	...
	public static function read_config() {
		...
		$c["sloway_site"] = (strpos("http", $paths->slowaySite) === 0) ? $c["sloway_site"] = $paths->slowaySite : site_url($paths->slowaySite);
		$c["media_site"] = (strpos("http", $paths->mediaSite) === 0) ? $c["media_site"] = $paths->mediaSite : site_url($paths->mediaSite);
		$c["uploads_site"] = (strpos("http", $paths->uploadsSite) === 0) ? $c["uploads_site"] = $paths->uploadsSite : site_url($paths->uploadsSite);
		$c["thumbs_site"] = (strpos("http", $paths->thumbsSite) === 0) ? $c["thumbs_site"] = $paths->thumbsSite : site_url($paths->thumbsSite);
