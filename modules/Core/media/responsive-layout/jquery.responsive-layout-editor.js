    ...
    $.rleditor = {
        ...
        menu: {
            ...
            copy_editable: function(id) {
                ...
                $(target).children(".rle_fragments").children(".rle_fragment").each(function() {
                    $.rleditor.output_fragment($(this), cb, 0, ops);    
                });
            ...
            copy_root: function(id) {               
                var edit = $("#" + id);
                if (!edit.hasClass("rl_editor"))
                    edit = edit.closest(".rl_editor");
                var ops = edit.data("rleditor");                                 
                
                var cb = edit.children(".rle_clipboard");
                cb.html("");
                
                edit.children(".rle_tree").children(".rle_fragments").children(".rle_fragment").each(function() {
                    $.rleditor.output_fragment($(this), cb, 0, ops);    
                });
                ...
                console.log(localStorage.getItem("rle_clipboard_html"));
            ...
            paste_root: function(id, name, pos_y) {  
                ...
                tmp.children("div").each(function() {
                    $.rleditor.load_template($(this), ul, 0, editor, ops, itm);
                });
            ...
            add_fragment: function(id, name) {  
                ...
                $.rleditor.create_resizers(edit, frag);
            ...
            add_fragment_root: function(id, name, pos_y) {  
                ...
                var frag = $.rleditor.create_fragment(editor, name, ops, ul, true, itm);
                $.rleditor.create_resizers(editor, frag);
        ...
        //  TL ELEMENT
            ...
            //  CONTAINER -> ADD TEMPLATES
                ...
                if (itm_container.items.length)
                    itm_container.items.push({separator: true});                
                ...
                if (target.hasClass("dyntree")) {    
                //  CONTAINER -> PASTE
                    var paste = localStorage.getItem("rle_clipboard_html");
                    if (paste) {
        ...
        context_menu: function(edit, target, x, y, ops, items) {
            ...
            var menu = target.contextmenu(x, y, items, {
                ...
                onClick: function(trg, name, x, y) {
                    ...
                    if ($(this).is(".rlm_root_copy"))
                        $.rleditor.menu.copy_root(id); else
                    if ($(this).is(".rlm_editable_paste"))
                        $.rleditor.menu.paste_fragment(id); else
                    if ($(this).is(".rlm_editable_copy"))
                        $.rleditor.menu.copy_editable(id); else                            
                    if ($(this).is(".rlm_root_paste"))
                        $.rleditor.menu.paste_root(id, name, y); else
        ...
        create_resizers: function(edit, fragment) {
            var temp = $("#rle_temp");
            if (!temp.length)
                temp = $("<div id='rle_temp'></div>").prependTo("body");

            var detach = (!fragment.is(":visible"));
            var parent = null;
            if (detach) {
                parent = fragment.parent();
                fragment.detach().appendTo(temp);
            }
            ...
            if (detach)
                fragment.detach().appendTo(parent);
        },
        ...
        load: function(edit, ops, reset_cid) {
            ...
            if (!ops.content_id || reset_cid) ops.content_id = "";
            ins.remove();
        ...
        load_template: function(template, parent, level, edit, ops, before) {
            ...
            $.rleditor.create_resizers(edit, fragment);      
        },      
        ...
        output: function(edit) {
            ...
            clearTimeout(ops.output_timeout);
            ops.output_timeout = setTimeout(function() {
        ...
        output_proc: function(edit) {
            ...
            if (!ops.content_id && ops.insert_cid) {
                if (!$.rleditor.instance_hash)
                    $.rleditor.instance_hash = new Date().getTime();
                
                ops.content_id = "content-" + $.rleditor.instance_hash + $.rleditor.instance_cnt;
                $.rleditor.instance_cnt++;
            }
        ...
        methods: {
            get_value: function(args) {
                var ok = (args.length == 1 && args[0] == "get_value");
                if (!ok) return false;
                
                return { result: $(this).find("textarea").val()};
            },
            set_value: function(args) {
                var ok = (args.length > 1 && args[0] == "set_value");
                if (!ok) return false;
                
                $(this).find("textarea").val(args[1]);
                $.rleditor.load($(this), $(this).data("rleditor"), true);
